...
fn is_array_like(t: &Type) -> bool {
    matches!(t, Type::Array(_) | Type::Iterable(_) | Type::List(_) | Type::Shape { .. })
}
...
fn run_invalid_cast(fa: &FileAnalysis) -> Vec<Diagnostic> {
    let mut out = Vec::new();
    walk::for_each_expr(fa.program, &mut |e| {
        let ExprKind::Cast { kind, expr } = &e.kind else { return };
...
fn run_echo(fa: &FileAnalysis) -> Vec<Diagnostic> {
    let mut out = Vec::new();
    walk::for_each_stmt(fa.program, &mut |s| {
        let StmtKind::Echo(exprs) = &s.kind else { return };
        for (i, ex) in exprs.iter().enumerate() {
            ...
            if never_string(&t) {
                out.push(
                    Diagnostic::error(
                        ex.span,
                        format!("Parameter #{} ({t}) of echo cannot be converted to string.", i + 1),
                    )
...
fn run_print(fa: &FileAnalysis) -> Vec<Diagnostic> {
    let mut out = Vec::new();
    walk::for_each_expr(fa.program, &mut |e| {
        let ExprKind::Print(inner) = &e.kind else { return };
...
pub(crate) static RULES: &[RuleEntry] = &[
    ...
    RuleEntry { name: "cast.invalid", level: 2, run: run_invalid_cast },
    RuleEntry { name: "cast.echo", level: 2, run: run_echo },
    RuleEntry { name: "cast.print", level: 2, run: run_print },
    RuleEntry { name: "cast.encapsedPart", level: 2, run: run_invalid_encapsed_part },
];
...
mod tests {
    ...
    fn cast_array_to_scalar_is_flagged() {
        assert_eq!(codes("<?php $x = (int) [];", run_invalid_cast), ["cast.int"]);
        assert_eq!(codes("<?php $x = (float) [1];", run_invalid_cast), ["cast.double"]);
        assert_eq!(codes("<?php $x = (string) [1, 2];", run_invalid_cast), ["cast.string"]);
    }
    ...
    fn cast_of_unknown_is_not_flagged() {
        assert!(codes("<?php function f($a) { return (int) $a; }", run_invalid_cast).is_empty());
    }
    ...
    fn array_in_interpolation_is_flagged() {
        ...
        assert_eq!(codes(src, run_invalid_encapsed_part), ["encapsedStringPart.nonString"]);
    }
